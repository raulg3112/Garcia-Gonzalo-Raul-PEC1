...
# Es converteix la información en matriu amb `as.matrix()`. Aquesta matriu es pot nombrar com matriu d'intensitat.
matriu_intensitat <- as.matrix(dades[ , -1])

# S'afegeixen el nom dels metabolits a cada fila i els noms de cada columna.
rownames(matriu_intensitat) <- dades[ , 1]
colnames(matriu_intensitat) <- colnames(dades[ , -1])
...
se <- SummarizedExperiment(assays = list(intensity = matriu_intensitat), colData = col_data)
